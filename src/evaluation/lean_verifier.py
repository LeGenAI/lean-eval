...
_LEAN_DIAG_RE = re.compile(
    r"^(?P<path>[^:\n]+):(?P<line>\d+):(?P<col>\d+):\s*"
    r"(?P<severity>error|warning|info):\s*(?P<body>.+?)$",
    flags=re.MULTILINE,
)
...
def parse_lean_messages(payload: str) -> List[LeanMessage]:
    ...
    Multi-line bodies (Lean's `error: ... \n     foo bar` style) are folded
    back into a single message body by collapsing leading whitespace lines
    onto the preceding diagnostic.
    """
    if not payload:
        return []
    messages: List[LeanMessage] = []
    for m in _LEAN_DIAG_RE.finditer(payload):
        messages.append(
            LeanMessage(
